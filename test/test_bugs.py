...
def test_bugs(bn=None):
  import os
  out_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'test_bugs', "mpl-" + matplotlib_version)
  ...
  for tn in tests.keys():
    ...
    for j, fig in enumerate(figs):
      if len(figs) == 1:
        now_file = os.path.join(out_dir, f'bugs_{tn:02d}.now.png')
      else:
        now_file = os.path.join(out_dir, f'bugs_{tn:02d}_{j+1:02d}.now.png')
      if 'format' in test and test['format'] == 'svg':
        now_file = now_file.replace('.png', '.svg')

      fig.savefig(now_file, bbox_inches='tight')
      print(f"Saved {now_file}")

      if not do_diff or ('format' in test and test['format'] == 'svg'):
        continue

      ref_file = now_file.replace('.now.', '.ref.')
      imgcheck(ref_file, now_file, show_diff=show_diff, generate_ref_files=generate_ref_files)
